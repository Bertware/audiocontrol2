...
class ScrollPhatHdDisplayThread(threading.Thread):
    ...
    def display_scrolling(self, scrolling_text, loops=1):
        ...
        # The font is 5x7. scrollphathd.scroll() moves all text one pixel
        # The total text length is 5 x length(scrolling_text)
        # The first 17 columns fit on the display without scrolling
        # So for each loop, call scroll '5 x length(scrolling_text) - 17' times
        # This means that, in order to scroll one loop, we need to scroll
        scroll_calls = 5 * len(scrolling_text) - 17
        if scroll_calls < 0:
            scroll_calls = 0
